...
## 1. `01_harness_onoff.py` ★ · 요소를 끄면 무너진다
...
읽기: 이번 실측은 양쪽 다 막았지만 성격이 다릅니다. 경계 없음의 방어는
...
## 5. `05_tool_permission.py` · 위험한 도구엔 게이트
...
읽기: 앞의 모든 방어가 뚫렸다고 가정한 최후의 방어선입니다. 인젝션이 모델을
넘어뜨렸어도 유출은 여기서 멈춥니다. 마지막 관문은 확률이 아니라 정규식,
결정적이어야 합니다.

## 7. `07_path_jail.py` · 파일 도구의 감금
...
읽기: 사람이 읽는 print가 아니라 기계가 다시 읽는 구조화 로그입니다. 어느
