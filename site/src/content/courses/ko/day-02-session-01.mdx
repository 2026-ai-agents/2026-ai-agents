...
시작점이고, v1.0은 완성된 제품입니다. `main`은 항상 v1.0입니다.
...
재시작하면 사라지고(→ booking-agent가 PostgreSQL로 해결), thread를 넘는
기억은 없으며(→ secretary-agent의 store), 수치는 45종 시드가 전부입니다
(→ food-rag-agent가 대규모 데이터와 검증으로 확장).
...
다음 세션: [diet-agent: LangGraph 기본기](/course/day-02-session-02/)
