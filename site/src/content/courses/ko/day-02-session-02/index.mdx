...
Day 1에서 같은 일을 하는 코드를 이미 짰습니다. 나란히 놓으면 차이가
분명해집니다.

```python
# Day 1 (day-01/agent/react.py) — 제어 흐름이 함수 '본문 안'에 있다
for step_no in range(1, max_steps + 1):
    response = completion(model=model, messages=messages, tools=tool_schemas())
    message = response.choices[0].message
    if not message.tool_calls:                 # 분기: if
        result.answer = message.content        # 종료: return
        return result
    for call in message.tool_calls:            # 도구 실행 + 되먹임
        observation = run_tool(…)
        messages.append({"role": "tool", …})   # 축적: append
```

```python
# diet-agent (app/agent/graph.py) — 제어 흐름이 코드 '밖'에 선언된다
...
| | 수제 루프 (Day 1) | 그래프 선언 (LangGraph) |
| --- | --- | --- |
| 흐름이 사는 곳 | 함수 본문 속. 읽어야 안다 | 선언부. 보면 안다 |
| 실행 주체 | 내가 쓴 while이 돈다 | 그래프 런타임이 노드를 부른다 |
