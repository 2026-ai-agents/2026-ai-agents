...
> 오릅니다.  
> ★ 표시는 수업 중 함께 실행하는 예제이고, 이 문서에는 **6종 전부의 실제
...
## 1. `01_plan_execute.py` ★ · 계획 먼저, 실행은 LLM 없이
...
## 2. `02_replan.py` ★ · 막히면 실패를 들고 계획으로 돌아간다
...
## 4. `04_eval_rubric.py` · 루브릭을 바꾸면 승자가 뒤집힌다
...
## 5. `05_rewoo.py` · 관찰 없이 플랜 한 번, 도구는 병렬
...
## 6. `06_loop_cost_compare.py` · 같은 과제, 두 루프의 계산서
