...
description: "왜 LiteLLM인가부터 examples/02_litellm 10종 전수 실행까지: 프로바이더 교체, 3사 비교, 폴백, 재시도, 캐싱, 비용 콜백, rate limit, 라우터, 임베딩"
...
## 1. 왜 LiteLLM인가: 공식 SDK 대신 통합 레이어

2세션의 예제들이 이미 `from litellm import completion`을 썼습니다. 각 사가
...
- **프로바이더 고유 기능의 지연·누수**: 새 기능은 litellm이 지원할 때까지
  기다리거나, 2세션 1번의 응답 원문에서 본 `provider_specific_fields`처럼
  표준 밖 필드로 새어 나옵니다. Gemini 3+의 temperature 은퇴를 경고로만
  전달하던 2세션의 temperature 예제도 같은 결입니다
