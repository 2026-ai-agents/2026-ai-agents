...
description: "examples/01_api 11종 전수 실행: 요청·응답 해부, 역할, 왜 LiteLLM인가, 토큰과 비용, 한도 초과, 스트리밍, 병렬, 무상태, 기억과 compact, JSON 강제, 비전"
...
## 3. 왜 LiteLLM인가: 공식 SDK 대신 통합 레이어

앞의 두 예제에서 이미 `from litellm import completion`을 썼습니다. 각 사가
공식 SDK를 제공하는데도 이 랩이 [[litellm|LiteLLM]] 하나로 통일한 이유를
여기서 정리하고 갑니다.
...
- **프로바이더 고유 기능의 지연·누수**: 새 기능은 litellm이 지원할 때까지
  기다리거나, 1번 실행 결과에서 본 `provider_specific_fields`처럼 표준 밖
  필드로 새어 나옵니다. Gemini 3+의 temperature 은퇴를 경고로만 전달하던
  6번 예제도 같은 결입니다
...
- 이 과정은 3사를 오가며 비교하는 것 자체가 커리큘럼이므로 LiteLLM입니다.
  폴백·라우터·캐시 같은 실전 장비는 [3세션](/course/day-01-session-03/)에서
  전부 돌려 봅니다
...
## 10. 기억의 정체: 채팅이 나를 기억하는 이유와 compact

9번에서 서버가 아무것도 기억하지 않는 것을 확인했습니다. 그런데 우리가 쓰는
채팅 서비스는 분명히 나를 기억합니다. 이 간극을 메우는 장치가 전부
messages 배열의 관리 기법입니다.

#### 채팅이 항상 나를 기억하는 이유

- **같은 대화 안에서의 기억**: 9번 그대로입니다. 클라이언트가 대화 전체를
...
- **대화를 새로 열어도 남는 기억**: ChatGPT의 메모리, Claude의 프로젝트
  ...
  Day 2 secretary-agent의 [[store]]가 정확히 이 구조입니다
...
- **한도**: 5번에서 본 그대로, 넘치는 순간 요청 자체가 거부됩니다
- **비용**: 13번의 산수대로, 입력 토큰은 매 턴 전체를 다시 냅니다
...
5세션에서 우리 에이전트의 컨텍스트가 스텝마다 부푸는 것을 실측하는데, 그
곡선의 끝에서 만나는 해법이 바로 이 구조입니다.

## 11. JSON 강제: 부탁이 아니라 계약으로
